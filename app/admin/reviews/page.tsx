  ...
  const [reviews, setReviews] = useState([
    {
      ...
      rating: 5,
      content: 'Amazing essay writing service! My paper was well-researched, properly formatted, and delivered on time.',
      date: '2024-01-15',
      verified: true,
    },
    {
      ...
      rating: 5,
      content: 'Outstanding thesis assistance. The team understood my research perfectly and delivered exceptional results.',
      date: '2024-01-14',
      verified: true,
    },
    {
      ...
      rating: 4,
      content: 'Great proofreading service. My assignment was polished to perfection with incredible attention to detail.',
      date: '2024-01-13',
      verified: true,
    },
  ]);
        ...
        <div className="sticky top-0 z-40 bg-gradient-to-r from-slate-900/95 to-slate-800/95 backdrop-blur border-b border-white/10 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-foreground/60 text-sm mt-1">Manage customer reviews and ratings</p>
            </div>
            <button
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="glass p-6 rounded-2xl border border-white/10">
              ...
              <h3 className="text-3xl font-bold text-foreground">{reviews.length}</h3>
            </div>
            <div className="glass p-6 rounded-2xl border border-white/10">
              <p className="text-foreground/70 text-sm mb-2">Average Rating</p>
              <div className="flex items-center gap-2">
                <h3 className="text-3xl font-bold text-yellow-400">{avgRating}</h3>
                <span className="text-2xl">⭐</span>
              </div>
            </div>
            <div className="glass p-6 rounded-2xl border border-white/10">
              <p className="text-foreground/70 text-sm mb-2">Verified Reviews</p>
              <h3 className="text-3xl font-bold text-green-400">{reviews.filter((r) => r.verified).length}</h3>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {filteredReviews.map((review) => (
              <div key={review.id} className="glass p-6 rounded-2xl border border-white/10 hover:border-white/20 transition-all">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-foreground/60 mt-1">{review.service}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {[...Array(5)].map((_, i) => (
                      <Star
          ...
          <div className="w-full max-w-2xl glass rounded-2xl border border-white/10 p-8">
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground/80 mb-2">Customer Name</label>
                  <input
                    type="text"
                    value={formData.name}
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground/80 mb-2">Service</label>
                  <select
              ...
              <div>
                <label className="block text-sm font-medium text-foreground/80 mb-2">Rating</label>
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
              ...
              <div>
                <label className="block text-sm font-medium text-foreground/80 mb-2">Review Content</label>
                <textarea
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label className="text-sm font-medium text-foreground/80">Mark as Verified Review</label>
              </div>
            </div>
