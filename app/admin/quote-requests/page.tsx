...
import { Send, MessageSquare, Trash2, Search, AlertCircle, CheckCircle2 } from 'lucide-react';
  ...
  const [requests, setRequests] = useState([
    {
      ...
      message: 'I need help writing a 5000-word essay on World War II. Can you provide a quote?',
    ...
    },
    {
      ...
      message: 'Looking for thesis writing assistance. Need pricing for a 80-page MBA thesis on organizational change.',
    ...
    },
    {
      ...
      message: 'I need a research paper on climate science. Can you provide information about your services and pricing?',
    ...
    },
    {
      ...
      message: 'We are looking for ongoing academic writing support. Can we discuss pricing for bulk orders?',
        ...
        <div className="flex-1 flex overflow-hidden">
              ...
              {filteredRequests.map((req) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-semibold text-foreground text-sm">{req.senderName}</h3>
                  ...
                  <p className="text-xs text-foreground/60 truncate">{req.subject}</p>
                  <p className="text-xs text-foreground/50 mt-1 truncate">{req.message}</p>
                  <p className="text-xs text-foreground/40 mt-2">{req.timestamp}</p>
                  {!req.replied && (
          ...
          {currentRequest ? (
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-6 border-b border-white/10 bg-gradient-to-r from-slate-900/50 to-slate-800/50">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h2 className="text-xl font-bold text-foreground">{currentRequest.senderName}</h2>
                    <p className="text-sm text-foreground/60">{currentRequest.senderEmail}</p>
                  </div>
                ...
                <p className="text-sm text-foreground/70 font-medium">{currentRequest.subject}</p>
              </div>

              {/* Request Message */}
              <div className="flex-1 overflow-y-auto p-6">
                <div className="glass bg-white/10 rounded-lg p-6 mb-6">
                  <h3 className="text-sm font-semibold text-foreground/80 mb-3">Customer Message</h3>
                  <p className="text-foreground/70 leading-relaxed whitespace-pre-wrap">{currentRequest.message}</p>
                  <p className="text-xs text-foreground/50 mt-4">Received: {currentRequest.timestamp}</p>
                </div>
              </div>
              ...
              {!currentRequest.replied ? (
                <div className="p-6 border-t border-white/10 bg-gradient-to-r from-slate-900/50 to-slate-800/50">
                  <div className="space-y-3">
                    <label className="block text-sm font-medium text-foreground/80">Send Quote Response</label>
                    <textarea
                      value={replyText}
                ...
                <div className="p-6 border-t border-white/10 bg-gradient-to-r from-green-900/20 to-emerald-900/20">
                  <div className="flex items-center gap-3 text-green-400">
                    ...
                    <span className="text-sm font-medium">Response sent to customer</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex-1 flex items-center justify-center">
              <p className="text-foreground/50">Select a quote request to view details</p>
            </div>
          )}
        </div>
      </div>
