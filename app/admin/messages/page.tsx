...
export default function AdminMessages() {
  ...
  const handleMarkResolved = (id: number) => {
    setConversations(
      conversations.map((c) => (c.id === id ? { ...c, resolved: !c.resolved } : c))
    );
  };
      ...
      <div className="flex-1 ml-64 flex flex-col overflow-hidden">
        ...
        <div className="sticky top-0 z-40 bg-gradient-to-r from-slate-900/95 to-slate-800/95 backdrop-blur border-b border-white/10 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-foreground">Messages & Chat</h1>
              ...
              {filteredConversations.map((conv) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <h3 className="font-semibold text-foreground text-sm">{conv.senderName}</h3>
                    {conv.unread && (
                  ...
                  <p className="text-xs text-foreground/50 mt-1 truncate">{conv.lastMessage}</p>
                  <p className="text-xs text-foreground/40 mt-2">{conv.timestamp}</p>
                  {!conv.resolved && (
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="p-6 border-b border-white/10 bg-gradient-to-r from-slate-900/50 to-slate-800/50">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    <h2 className="text-xl font-bold text-foreground">{currentConversation.senderName}</h2>
                    <p className="text-sm text-foreground/60">{currentConversation.senderEmail}</p>
                  </div>
                ...
                <p className="text-sm text-foreground/70 font-medium">{currentConversation.subject}</p>
              </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <p className={`text-xs mt-2 ${
                          msg.sender === 'admin' ? 'text-white/70' : 'text-foreground/60'
                        }`}>
                          {msg.time}
                        </p>
                      </div>
